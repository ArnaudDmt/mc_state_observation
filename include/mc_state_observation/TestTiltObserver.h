...
{
struct TestTiltObserver : public mc_observers::Observer
{
...
public:
  /// @brief Constructor for the TestTiltObserver.
  /// @details The parameter is given only if the Tilt Observer is used as a backup by the Kinetics Observer
  TestTiltObserver(const std::string & type, double dt, bool asBackup = false);
...
protected:
  ...
  stateObservation::kine::Kinematics poseWKine_; ///< Estimated pose of the floating-base in world frame */
